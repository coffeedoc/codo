(function() {
  ...
  module.exports = Doc = (function(_super) {
    ...
    function Doc(node, options) {
      ...
      try {
        if (this.node) this.parseTags(this.node.comment.split('\n'));
      } catch (error) {
    ...
    Doc.prototype.parseTags = function(lines) {
      var abstract, author, code, comment, concern, copyright, deprecated, doc, example, extend, include, innerComment, line, method, mixin, note, option, overload, param, returnValue, see, signature, since, text, title, todo, version, _base, _name, _ref;
      comment = [];
      while ((line = lines.shift()) !== void 0) {
        ...
        if (returnValue = /^@return\s+\[(.+?)\](?:\s+(.+))?/i.exec(line)) {
          ...
          if (innerComment.length !== 0) {
            ...
            this.overloads.push({
              signature: signature.replace(/([$A-Za-z_\x7f-\uffff][$\w\x7f-\uffff]*)(.+)/, function(str, name, params) {
                return "<strong>" + name + "</strong>" + params;
              }),
              comment: doc.comment,
              summary: doc.summary,
              params: doc.params,
      ...
      return this.comment = marked(text).replace(/\n+/g, ' ');
    };
